...
use DateTimeImmutable;use PDO;use RuntimeException;use Throwable;
...
final class CloseExercise
{
    ...
    public function execute(string $startDate,int $userId):int
    {
        ...
        $counts->execute(['quote_cutoff'=>$startDate,'offer_cutoff'=>$startDate]);$counts=$counts->fetch();
        ...
        try{
            ...
            $run->execute(['start'=>$startDate,'previous'=>$previous,'offers'=>(int)$counts['offers'],'quotes'=>(int)$counts['quotes'],'user'=>$userId]);$runId=(int)$this->db->lastInsertId();
            $queries=[
            ...
            $quoteIds=$this->db->prepare('SELECT d.id FROM devis d JOIN offre o ON o.id_offre=d.offre_id WHERE o.date_offre<:cutoff AND d.archived_at IS NULL');$quoteIds->execute(['cutoff'=>$startDate]);$quoteMap=array_flip(array_map('intval',$quoteIds->fetchAll(PDO::FETCH_COLUMN)));
