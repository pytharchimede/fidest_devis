...
final class NeedRequestPdf extends \FPDF
{
    ...
    public function renderDocument():void
    {
        ...
        $this->section('4. VISA ET MAÎTRISE DOCUMENTAIRE');$y=$this->GetY();$visas=[['Établi par','prepared_name','prepared_at','prepared_signature'],['Analysé par','analyzed_name','analyzed_at','analyzed_signature'],['Approuvé par','approved_name','approved_at','approved_signature']];foreach($visas as $i=>[$title,$nameKey,$dateKey,$signatureKey]){$x=14+$i*52;$this->SetXY($x,$y);$this->SetFont('Arial','B',7);$this->Cell(50,7,$this->t($title),1,2,'C');$this->SetFont('Arial','',6);$name=(string)($d[$nameKey]??'');$date=(string)($d[$dateKey]??'');$this->Cell(50,20,$this->t($name!==''?$name.($date!==''?' · '.date('d/m/Y H:i',strtotime($date)):''):'En attente de visa'),1,0,'C');$signature=(string)($d[$signatureKey]??'');$signatureFile=APP_ROOT.'/'.$signature;if($signature!==''&&is_file($signatureFile)){try{$this->Image($signatureFile,$x+15,$y+10,20,10);}catch(\Throwable){}}}$this->Image($this->qr,174,$y+1,20);$this->SetXY(168,$y+22);$this->SetFont('Arial','',5.5);$this->MultiCell(28,3,$this->t('Scanner pour vérifier le document'),0,'C');
    }
}
