...
$footerValue = trim((string) ($devis['pied_de_page'] ?? '')) ?: app_branding()->footerBlock();
...
<body class="quote-editor">
    ...
    <form id="devisForm" action="request/modif_devis.php" method="POST" enctype="multipart/form-data">
        ...
        <input type="hidden" id="devisId" name="devisId" value="<?= (int) $devis['id'] ?>">


        <div class="container mt-4">
        <div class="page-heading">
            <h1 class="text-center">Modifier le devis</h1>
            <p>Mettez à jour les informations et les lignes du document.</p>
        </div>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <select class="form-control" id="clientSelect" name="client_id" data-smart-select data-placeholder="Rechercher un client…">
                        ...
                        <?php while ($client = $clients->fetch(PDO::FETCH_ASSOC)): ?>
                            <?php $recipient = implode("\n", array_filter([
                                $client['nom_client'] ?? '', $client['localisation_client'] ?? '',
                                $client['commune_client'] ?? '', $client['bp_client'] ?? '', $client['pays_client'] ?? '',
                            ])); ?>
                            <option value="<?php echo $client['id_client']; ?>" data-recipient="<?= htmlspecialchars($recipient, ENT_QUOTES, 'UTF-8') ?>" <?php if ($devis['client_id'] == $client['id_client']) {
                                                                                    echo 'selected';
                                                                                } ?>>
                    ...
                    <a id="editSelectedClient" class="btn btn-sm btn-link px-0" href="modifier_client.php?id=<?= (int) $devis['client_id'] ?>"><i class="fas fa-pen"></i> Modifier le client sélectionné</a>
                </div>
                <div class="col-md-6">
            ...
            <div class="checkbox_zone">
                <div class="form-group">
                    ...
                    <label class="switch">
                        <input type="checkbox" id="tvaFacturable" name="tvaFacturable" value="1" <?= $devis['tva_facturable'] == 1 ? 'checked' : '' ?>>
                ...
                <div class="form-group">
                    ...
                    <label class="switch">
                        <input type="checkbox" id="publierDevis" name="publierDevis" value="1" <?= $devis['publier_devis'] == 1 ? 'checked' : '' ?>>
                        <span class="slider round"></span>
                    </label>
                </div>
            </div>
            ...
            <div class="d-flex align-items-start mb-4">
                ...
                <div class="info-container ms-3">
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" id="dateFacturation" name="dateFacturation" value="<?= htmlspecialchars((string) ($devis['date_facturation_prevue'] ?? '')) ?>">
                    </div>
                </div>
            </div>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <textarea class="form-control" id="emisPar" name="emisPar" rows="5" readonly><?= htmlspecialchars($issuer, ENT_QUOTES, 'UTF-8') ?></textarea>
                    <div class="form-text">Identité officielle FIDEST centralisée dans la charte de l’application.</div>
                </div>
                <div class="col-md-6">
                    <label for="destineA" class="form-label">Destinataire</label>
                    <textarea class="form-control" id="destineA" name="destineA" placeholder="Sélectionnez un client" rows="4" readonly><?= htmlspecialchars((string) $devis['destine_a'], ENT_QUOTES, 'UTF-8') ?></textarea>
                    <div class="form-text">Renseigné automatiquement depuis la fiche du client sélectionné.</div>
                </div>
            </div>
            ...
            <div class="row footer-info mt-4">
                <div class="col-md-8">
                    ...
                    <div class="form-group mt-3">
                        ...
                        <textarea class="form-control" id="piedDePage" name="piedDePage" rows="5" placeholder="Pied de page"><?= htmlspecialchars($footerValue, ENT_QUOTES, 'UTF-8') ?></textarea>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="form-group mt-3">
