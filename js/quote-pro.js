(function () {
    ...
    const money = amount => new Intl.NumberFormat('fr-FR', { maximumFractionDigits: 0 }).format(Number(amount) || 0) + ' FCFA';
    ...
    function addSectionTitle(element, icon, title) {
        ...
        element.insertAdjacentHTML('afterbegin', `<h2 class="editor-section-title"><span><i class="fas ${icon}"></i></span>${title}</h2>`);
    }
    ...
    function createRichEditor(textarea) {
        ...
        const sync = () => { textarea.value = editor.innerText.trim(); updatePreview(); };
        editor.addEventListener('input', sync);
        $$('.rich-toolbar button', wrapper).forEach(button => button.addEventListener('click', () => {
            editor.focus();
    ...
    function previewRows() {
        return $$('#devisTable tbody tr').map(row => {
            const description = $('[name="designation[]"]', row)?.value || 'Prestation';
    ...
    function updatePreview() {
        ...
        $('#previewIssuer').textContent = value('#emisPar') || 'Informations de votre entreprise';
        $('#previewRecipient').textContent = value('#destineA') || $('#clientSelect option:checked')?.textContent?.trim() || 'Client';
        ...
        $('#previewDelivery').textContent = value('#delaiLivraison') || 'À convenir';
        $('#previewContact').textContent = value('#correspondant') || 'Non renseigné';
        $('#previewExpiration').textContent = value('#dateExpiration') || 'Non renseignée';
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if ($('#devisId')) {
            $('#openPreview').innerHTML = '<i class="fas fa-file-pdf"></i> Enregistrer et voir le PDF exact';
        }

        const container = heading?.parentElement;
        addSectionTitle(container?.querySelector('.row.mb-3'), 'fa-address-book', 'Client et référence commerciale');
        addSectionTitle(container?.querySelector('.d-flex.align-items-start.mb-4'), 'fa-file-signature', 'Identité du document');
        addSectionTitle($('#devisForm > .row.mb-3'), 'fa-building', 'Émetteur et destinataire');
        addSectionTitle($('.footer-info'), 'fa-circle-check', 'Conditions et finalisation');

        const tableWrapper = $('#devisTable')?.closest('.table-responsive');
        if (tableWrapper && !tableWrapper.parentElement.classList.contains('line-items-section')) {
            ...
            section.insertAdjacentHTML('afterbegin', '<h2 class="editor-section-title"><span><i class="fas fa-layer-group"></i></span>Prestations et produits</h2>');
        }
        ...
        const syncRecipient = () => {
            ...
            recipientField.value = clientSelect.selectedOptions[0]?.dataset.recipient || '';
            updatePreview();
        };
        ...
        bindEditLink($('#offreSelect'), $('#editSelectedOffer'), 'modifier_offre.php');
        ...
        $('#previewBackdrop')?.addEventListener('click', () => togglePreview(false));
        document.addEventListener('keydown', event => { if (event.key === 'Escape') togglePreview(false); });
        document.addEventListener('input', updatePreview);
        document.addEventListener('change', updatePreview);
        document.addEventListener('click', event => {
            if (event.target.closest('#addRow, .remove-row')) setTimeout(updatePreview, 0);
        });
    ...
    document.addEventListener('quote:saved', () => notify('Le devis a été enregistré avec succès.', 'success'));
    document.addEventListener('quote:error', () => notify('L’enregistrement a échoué. Vérifiez les informations saisies.', 'error'));
})();
