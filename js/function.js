$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (exactPreviewWindow) {
                    exactPreviewWindow.location.href = 'request/preview_pdf.php?devisId=' + encodeURIComponent($('#devisId').val());
                    exactPreviewWindow = null;
                }
            },
            error: function(xhr, status, error) {
                console.error('Erreur:', status, error);
                document.dispatchEvent(new CustomEvent('quote:error'));
            }
        });
    });
    ...
    document.addEventListener('quote:request-exact-preview', function() {
        ...
        if (exactPreviewWindow) exactPreviewWindow.document.body.innerHTML = '<p style="font-family:sans-serif;padding:24px">Mise à jour du devis et génération de l’aperçu PDF…</p>';
        $('#saveBtn').trigger('click');
    });

    $("#exportPdfBtn").on('click', function(){
        var id = $('#devisId').val();
        $(location).attr('href', 'request/export_pdf.php' + (id ? '?devisId=' + encodeURIComponent(id) : ''));
    });
