$(document).ready(function () {
  ...
  $(document).on("input", ".prix, .quantite, .tva, .remise", function () {
    ...
    let totalHTLine = prix * quantite * (1 - remise / 100);
    let total = totalHTLine + totalHTLine * (tva / 100);
  ...
  function calculateTotals() {
    ...
    $("#devisTable tbody tr").each(function () {
      ...
      // La remise s’applique à la base HT avant le calcul de la TVA.
      let totalHTLine = prix * quantite * (1 - remise / 100);

      // Calculer le total TTC pour cette ligne en fonction de la TVA facturable
      let totalTTCLine;
      if (tvaFacturable) {
        totalTTCLine = totalHTLine + totalHTLine * (tva / 100);
        tvaTotal += totalHTLine * (tva / 100); // Accumuler le total de la TVA
      } else {
        totalTTCLine = totalHTLine; // TVA non facturable, on n’ajoute pas la TVA
      }
